...
class FirebaseServiceTests: XCTestCase {
    ...
    func testPerformanceOfFetchWorks() {
        measure {
            ...
            FirebaseService.shared.fetchWorks().retry(3).timeout(timeout: 30.0).then({ works in
            ...
            wait(for: [fetchWorksExpectation], timeout: 30.0)
        }
    }
    ...
    func testPerformanceOfDownloadResource() {
        measure {
            ...
            FirebaseService.shared.download(resource: resourceName, to: resourceDirectoryToSave).retry(3).timeout(timeout: 30.0).then({ resourcePath in
            ...
            wait(for: [downloadResourceExpectation], timeout: 30.0)
        }
    }
    ...
    func testPerformanceOfDownloadImage() {
        measure {
            ...
            FirebaseService.shared.download(image: imageName, to: imagesDirectoryToSave).retry(3).timeout(timeout: 30.0).then({ imagePath in
            ...
            wait(for: [downloadImageExpectation], timeout: 30.0)
        }
    }
}
